...
def draw_info(data_db, draw_info, image, day_month, user_id):
    dates_this_week = [(value, timestamp) for value, timestamp in data_db if
                       is_this_week(timestamp)]
    dates_yesterday = [(value, timestamp) for value, timestamp in data_db if
    ...
    if all_rate and today_rate and week_rate:
        ...
        if yesterday_rate:
            yesterday_avg = round(sum(yesterday_rate) / len(yesterday_rate), 2)
        ...
        if yesterday_avg:
            percent_today = md.calculate_percentage(today_avg, yesterday_avg)
            percent_week = md.calculate_percentage(today_avg, week_minis_today)
        ...
        width_all = get_text_width(f"{all_avg} /", font)
        width_week = get_text_width(f"{week_avg} /", font)
        width_today = get_text_width(f"{today_avg} /", font)
        ...
        draw.text((width_all + 411 + 9, 95), f"{percent_all[0]}", font=font,
                  fill=percent_all[1])
        draw.text((width_week + 448 + 9, 181), f"{percent_week[0]}", font=font,
                  fill=percent_week[1])
...
def draw_new_post(x, x2, y, image, rate):
    draw = ImageDraw.Draw(image)
    ...
    if last_30_days:
        ...
        for date in last_30_days:
            ...
            image.paste(rotated_text_image, (main_image_x, main_image_y),
                        rotated_text_image)
            image = draw_new_post(post_x, post_x + 29,
                                  post_y - round(50 * (rate - 1)), image, rate)
            main_image_x += 53
            post_x += 53
        draw = ImageDraw.Draw(image)
        draw.line((90, 930, 1700, 930), fill='black', width=2)
        image = draw_info_30(last_30_days, image, user_id)
        image.save(f'img/{user_id}_month.png')


def draw_day(day_month, user_id):
    try:
        data_db = db.data_to_draw(user_id)
        image = Image.open('img/background.png').convert("RGBA")
        days = int((time.time() - md.str_to_time(day_month)) / 86400)
        data_draw = db.data_date(user_id, days)
        for i in data_draw:
            hour = time.localtime(i).tm_hour - 6
            rate = data_draw[i]['rate']
            x = 100 * hour
            y = 883 - 50 * (rate - 1)
            image = draw_new_post(x, x + 50, y - 1, image, rate)
            draw = ImageDraw.Draw(image)
            draw.line((90, 930, 1700, 930), fill='black', width=2)

        draw_info(data_db, data_draw, image, day_month, user_id)
